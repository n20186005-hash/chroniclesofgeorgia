    ...
    <div className="min-h-screen bg-[#FDFCF8] dark:bg-[#0a0a0a] text-[#1a1a1a] dark:text-[#f0f0f0] font-sans selection:bg-black selection:text-white dark:selection:bg-white dark:selection:text-black transition-colors duration-300">
        ...
        <div className="max-w-[90rem] mx-auto px-6 lg:px-12">
          <div className="flex justify-between items-center h-24">
            ...
            <div className="hidden lg:flex items-center space-x-4 text-[11px] font-bold tracking-[0.2em] uppercase text-black/40 dark:text-white/40">
              ...
              <span>/</span>
              <span className="text-black dark:text-white">{tLegal('privacy')}</span>
            </div>
      ...
      <main className="py-24 md:py-40 min-h-[70vh]">
        <article className="max-w-3xl mx-auto px-6 lg:px-8">
          ...
          <div className="prose prose-lg md:prose-xl dark:prose-invert prose-gray mx-auto prose-p:leading-[2] prose-p:text-black/80 dark:prose-p:text-white/80 prose-p:font-light prose-h2:font-serif prose-h2:text-3xl prose-h2:font-normal prose-h2:mt-16 prose-h2:mb-8 prose-h2:text-black dark:prose-h2:text-white">
              ...
              if (/^\d+\./.test(paragraph)) {
                ...
                return (
                  <div key={index} className="mb-12">
                    <h2 className="border-t border-black/10 dark:border-white/10 pt-8">{heading}</h2>
                    {textParts.length > 0 && <p>{textParts.join('\n')}</p>}
                  </div>
                );
              }
              return <p key={index} className="mb-10">{paragraph}</p>;
            })}
          </div>
